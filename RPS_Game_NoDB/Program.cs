        ...
        static void Main(string[] args)
        {
            ...
            do//game loop
            {
                ...
                while (game.winner.Name == "null")
                {
                    ...
                    //get the choices for the computer
                    //insert the computers choices directly into the round
                    round.ComputerChoice = (Choice)rand.Next(3); //this will give a random number starting at 0 to arg-1;
                    
                    round.p1Choice = new Choice();

                    bool isint; //create new bool value 'isint'
                    int newint; //create new int 'newint'
...

                    {
                        ...
                        System.Console.WriteLine("Please enter '0' for Rock, '1' for Paper, '2' for Scissors"); //prompt user to enter choice
                        string PlayerChoice = System.Console.ReadLine(); //read user input as string
                        isint = int.TryParse(PlayerChoice, out newint); //parse string as int, or return false
                        round.p1Choice = (Choice)newint; //set newint to user's choice
                      
                                   
                    } while (!isint || ((newint > 2) || newint < 0)); //iterate through loop until user's choice equals 1,2, or 3
                                     
                    

                    

                    //round.p1Choice = (Choice)rand.Next(3);
